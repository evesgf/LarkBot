/*---------------------------------------------------------------
 * 作者：evesgf    创建时间：2016-8-2 10:04:36
 * 修改：evesgf    修改时间：2016-8-2 10:04:40
 *
 * 版本：V0.0.2
 * 
 * 描述：消息通知中心
 * NotificationCenter用于处理GameObjects之间的消息传递。
 * GameObjects能够注册和接收具体的消息通知，另一个该类型的对象发
 * 送一个通知，其他所有注册过的对象都能接收到消息。
 * 
 * 参考：
 * 官方消息中心：http://blog.csdn.net/pleasecallmewhy/article/details/8534349
 * 凉鞋简易消息机制：http://liangxiegame.com/unity-you-xi-kuang-jia-da-jian-wu-jian-yi-xiao-xi-ji-zhi/
 * 
 * TODO:消息中心修改须知
 * 1、消息中心需要和单例整合
 * 1.目前整个游戏的消息都由一个字典维护,可以改进为每个模块维护一
 * 个字典或者其他方式。
 * 2.消息名字类型由字符串定义的,可以改成枚举转unsigned int方式。
 * 坑
 * 1.如果是MonoBehaviour注册消息之后,GameObject Destroy之前一定
 * 要注销消息,之前的解决方案是,自定义一个基类来维护该对象已经注册
 * 的消息列表,然后在基类的OnDestory时候遍历卸载。
 * 
 * Observing GameObjects must register to receive notifications
 * with the AddObserver function, and pass their selves, and the
 * name of the notification.  Observing GameObjects can also 
 * unregister themselves with the RemoveObserver function. 
 * GameObjects must request to receive and remove notification 
 * types on a type by type basis.
 * Posting notifications is done by creating a Notification object
 * and passing it to PostNotification.  All receiving GameObjects 
 * will accept that Notification object.  The Notification object 
 * contains the sender, the notification type name, and an option 
 * hashtable containing data.
 * To use NotificationCenter, either create and manage a unique 
 * instance of it somewhere, or use the static NotificationCenter.
 * We need a static method for objects to be able to obtain the 
 * default notification center.
 * This default center is what all objects will use for most 
 * notifications.  We can of course create our own separate 
 * instances of NotificationCenter, but this is the static one 
 * used by all.
 ---------------------------------------------------------------*/
...
namespace LarkFramework
{
    public class NotificationCenter : MonoBehaviour
    {
        ...
        public static NotificationCenter DefaultCenter()
        {
            ...
            if (!defaultCenter)
            {
                // 因为通知中心是个组件，所以得创建个GameObject并添加到场景里
        ...
        //们的哈希表包含所有通知。每个通知的哈希表是一个ArrayList,包含所有的观察家通知。
        ...
        //TODO:AddObserver包括一个版本的观察者只能请求从一个特定的对象接收通知。我们还没有实现,所以现在发件人值被忽略。
